...
void get_stats_grid(int* rep, int* L, int* indexInfestInit, int* endIndex, int* grid.nbStats, int* numDiffGrids, int* gridIndexes, int* gridNumCells, int* gridEmptyCells, int* gridCountCells, double* grid.stats){

	double* stats = *(grid.stats + *rep * *grid.nbStats);
	...
	// need to compute variance + store in grid.stats
	...
	for(int grid=0; grid<*numDiffGrids; grid++){
		...
		for(int cell=0; cell<*(gridNumCells+grid); cell++){
			...
			var_percentpositive += gridEmptyCells[count]/gridNumCells[count] * gridEmptyCells[count]/gridNumCells[count];
		}
		...
		//store positive count in grid.stats
	 	...
	 	if(*getStats==1){
			...
			for(int stat=0;stat<*lengthStats; stat++){
				...
				switch(matchStats[stat]){
					...
					case 2: get_stats_grid(&rep, L, indexInfestInit, endIndex, grid.nbStats, numDiffGrids, gridIndexes, gridNumCells, gridEmptyCells, gridCountCells, grid.stats); break;
