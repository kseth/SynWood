...
{# 31-12-1997: simple fit of MSN distribution using moments
  y     <- as.matrix(y)
  k     <- ncol(y)
  m.y   <- apply(y,2,mean)
  var.y <- var(y)
...
msn.moment.fit.modified <- function(y)
{# 31-12-1997: simple fit of MSN distribution using moments
...
## Function by KS
msn.mle.modified <-function(y, start, trace=FALSE, 
                algorithm=c("nlminb", "Nelder-Mead", "BFGS", "CG",  "SANN"),
                control=list())
{
  t1 <- Sys.time()
  if(class(y) == "data.frame") y <- data.matrix(y)
  ...
  y.names<-dimnames(y)[[2]] 
  x.names<-dimnames(X)[[2]]
  if(missing(start)){	#initial estimation procedure to provide a good start for the MLE estimation of parameters
      fit0  <- lm.fit(X, y, method="qr")
      beta  <- as.matrix(coef(fit0))
      res   <- resid(fit0)
      a     <- msn.moment.fit.modified(res)
      Omega <- a$Omega
      omega <- a$omega
      alpha <- a$alpha
      if(!a$admissible) alpha<-alpha/(1+max(abs(alpha))) ##fit of the moments may result in skewness higher than allowed - correct for skewness issue
      beta[1,] <- beta[1,]-omega*a$delta*sqrt(2/pi)  
     }
  else{
    beta  <- start$beta
  ...
  list(call=match.call(), dp=dp, logL=logL, algorithm=opt, times=list(initTime=difftime(t2, t1, unit="secs"), fitTime=difftime(t3, t2, unit="secs"), total=difftime(t3, t1, unit="secs")))
}
